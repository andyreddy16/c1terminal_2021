...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state):
        ...
        if self.add_supports:
            ...
            if game_state.get_resource(MP, 0) > 9 and ( game_state.turn_number % 10 not in [0, 1, 2, 3] or game_state.my_health < 10 ):
    ...
    def add_walls_to_turrets(self, turret_locations, game_state):
        ...
        for loc in turret_locations:
            # Avoid placing a wall within 1 pos of the edge, so that we don't block attacks
            if loc[0] <= 13 and loc[0] + loc[1] <= 15:
                wall_loc = [[loc[0], loc[1] + 2], [loc[0] + 1, loc[1] + 2]]
            else:
    ...
    def build_reactive_defense(self, game_state):
        ...
        for location in self.scored_on_locations:
            # We want to avoid trapping our scouts that spawn at [13,0]
            # so build turrets up and in one position
            if location[0] <= 13:
